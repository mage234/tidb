...
func DecodeDecimal(b []byte) ([]byte, types.Datum, int, error) {
	var d types.Datum
	if len(b) < 3 {
		return b, d, 0, errors.New("insufficient bytes to decode value")
	}
	precision := int(b[0])
	frac := int(b[1])
	b = b[2:]
	...
	b = b[binSize:]
	if err != nil {
		return b, d, 0, errors.Trace(err)
	}
	d.SetLength(precision)
	d.SetFrac(frac)
	d.SetMysqlDecimal(dec)
	return b, d, 2 + binSize, nil
}
